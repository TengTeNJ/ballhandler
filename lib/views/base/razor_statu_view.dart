...
class RazoraStatuView extends StatefulWidget {
  const RazoraStatuView({super.key});

  @override
  State<RazoraStatuView> createState() => _RazoraStatuViewState();
}

class _RazoraStatuViewState extends State<RazoraStatuView> {
  ...
  void initState() {
    ...
    subscription = EventBus().stream.listen((event) {
      if (event == kCurrentDeviceInfoChange) {
        ...
        if (model.deviceName.contains(kThreeBallHandler_Name)) {
          getBatteryAndBlestatuValues();
        }
      ...
      }else if(event == kInitiativeDisconnectThree || event == kCurrentDeviceDisconnectedThree){
        connected = false;
