...
class StatsDetailPageController extends StatefulWidget {
  const StatsDetailPageController({super.key});

  @override
  State<StatsDetailPageController> createState() => _StatsDetailPageControllerState();
}

class _StatsDetailPageControllerState extends State<StatsDetailPageController> {
        ...
        child: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              Constants.boldWhiteTextWidget('My Trend', 30),
              SizedBox(
                height: 28,
              ),
              // grid view
              ...
              Row(
                ...
                children: [
                  Constants.regularWhiteTextWidget('Best of the Day', 14),
                  GestureDetector(
              ...
              Container(
                height: 8 * 36 + 36 + 20,
                child: LineAreaView(datas, context, selectType: _selectType),
              ),
              SizedBox(
                height: 40,
              ),
              Constants.regularWhiteTextWidget('My Top 20', 14),
              SizedBox(
                height: 16,
              ),
              Container(
                height: 8 * 36 + 36 + 32,
                child: BarView(barViewDatas, context, selectType: _selectType),
              ),
            ],
          ),
        ),
...
/*折线图*/
Widget LineAreaView(List<MyStatsModel> datas, BuildContext context,
    {int selectType = 1}) {
  int page = 1;
  if (datas.length > 10) {
    page = (datas.length / 10).ceil();
  }
  ...
  //         datas: datas.sublist(
  //             index * 10,
  //             (index + 1) * 10 > datas.length
  //                 ? datas.length
  //                 : (index + 1) * 10),selectType: selectType,);
  //   },
  //   itemCount: page,
  // );
  return ListView(
    ...
    children: [
      SizedBox(
        width: (Constants.screenWidth(context) - 32) * page,
    ...
    itemBuilder: (context, index) {
      return MyStatsLineAreaView(
        datas: datas.sublist(index * 10,
            (index + 1) * 10 > datas.length ? datas.length : (index + 1) * 10),
        selectType: selectType,
      );
    },
    itemCount: page,
...
/*柱状图*/
Widget BarView(List<MyStatsModel> datas, BuildContext context,
    {int selectType = 1}) {
  // if(datas.length == 0){
  //   return NoDataView();
  // }
  ...
  //         datas: datas,selectType: selectType,);
  //   },
